...
def test_ttest():
    ...
    for i,(dataset,zctol) in enumerate( zip(datasets, zctols) ):
        ...
        spm     = spm1d.stats.ttest(y, mu).inference(alpha, two_tailed=two_tailed)
...
def test_ttest_paired():
    ...
    for i,(dataset,zctol) in enumerate( zip(datasets, zctols) ):
        ...
        spm     = spm1d.stats.ttest_paired(y1, y0).inference(alpha, two_tailed=two_tailed)
        snpm    = spm1d.stats.nonparam.ttest_paired(y1, y0)
        niter   = -1 if snpm.nPermUnique < 10000 else 1000
        snpm    = snpm.inference(alpha, two_tailed=two_tailed, iterations=niter)
        assert spm.z == pytest.approx(snpm.z, abs=1e-4)
        assert spm.zstar == pytest.approx(snpm.zstar, abs=zctol)
...
def test_ttest2():
    ...
    two_tailed  = True
    datasets = []
    datasets.append(  spm1d.data.uv1d.t2.SmallSampleLargePosNegEffects()  )
    zctols      = [0.3]
    for i,(dataset,zctol) in enumerate( zip(datasets, zctols) ):
        np.random.seed(i)
        y0,y1   = dataset.get_data()
        spm     = spm1d.stats.ttest2(y1, y0).inference(alpha, two_tailed=two_tailed)
        snpm    = spm1d.stats.nonparam.ttest2(y1, y0)
        niter   = -1 if snpm.nPermUnique < 10000 else 1000
        snpm    = snpm.inference(alpha, two_tailed=two_tailed, iterations=niter)
        assert spm.z == pytest.approx(snpm.z, abs=1e-4)
        assert spm.zstar == pytest.approx(snpm.zstar, abs=zctol)
...
def test_regress():
    ...
    for i,(dataset,zctol) in enumerate( zip(datasets, zctols) ):
        ...
        spm     = spm1d.stats.regress(y, x).inference(alpha, two_tailed=two_tailed)
