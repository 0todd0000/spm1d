...
def test_ttest():
    ...
    for dataset in datasets:
        ...
        spm     = spm1d.stats.c.ttest(y, mu).inference(0.05, two_tailed=False)
...
def test_ttest_paired():
    ...
    for dataset in datasets:
        ...
        y       = np.hstack( [y0,y1] )
        x       = np.asarray( [0]*y0.size + [1]*y1.size )
        spm     = spm1d.stats.c.ttest_paired(y, x).inference(0.05, two_tailed=False)
        assert dataset.z == pytest.approx(spm.z, abs=1e-4)
        assert dataset.df == pytest.approx(spm.df, abs=1e-4)
        assert dataset.p == pytest.approx(spm.p, abs=1e-4)
...
def test_ttest2():
    ...
    for dataset in datasets:
        ...
        y       = np.hstack( [y0,y1] )
        x       = np.asarray( [0]*y0.size + [1]*y1.size )
        spm     = spm1d.stats.c.ttest2(y, x, equal_var=True).inference(0.05, two_tailed=False)
        assert dataset.z == pytest.approx(spm.z, abs=1e-4)
        assert dataset.df == pytest.approx(spm.df, abs=1e-4)
        assert dataset.p == pytest.approx(spm.p, abs=1e-4)
...
def test_regress():
    ...
    for dataset in datasets:
        ...
        spm     = spm1d.stats.c.regress(y, x).inference(0.05, two_tailed=True)
        assert dataset.z == pytest.approx(spm.z, abs=1e-4)
        assert dataset.df == pytest.approx(spm.df, abs=1e-4)
        assert dataset.p == pytest.approx(spm.p, abs=1e-4)
...
def test_anova1():
    ...
    for dataset in datasets:
        ...
        spm     = spm1d.stats.c.anova1(y, A, equal_var=True).inference(0.05)
...
def test_anova1rm():
    ...
    for dataset in datasets:
        ...
        x       = np.vstack( [A,S] ).T
        spm     = spm1d.stats.c.anova1rm(y, x, equal_var=True).inference(0.05)
        assert dataset.z == pytest.approx(spm.z, abs=1e-2)
        assert dataset.df == pytest.approx(spm.df, abs=1e-4)
        _assert_p(dataset.p, spm.p, tol=1e-3)
...
def test_anova2():
    ...
    for dataset in datasets:
        ...
        x       = np.vstack( [A,B] ).T
        spms    = spm1d.stats.c.anova2(y, x, equal_var=True).inference(0.05)
...
def test_anova2nested():
    ...
    for dataset in datasets:
        y,A,B   = dataset.get_data()
        x       = np.vstack( [A,B] ).T
        spms    = spm1d.stats.c.anova2nested(y, x, equal_var=True).inference(0.05)
        for spm,z,df,p in zip(spms, dataset.z, dataset.df, dataset.p):
            assert z == pytest.approx(spm.z, abs=1e-2)
            assert df == pytest.approx(spm.df, abs=1e-4)
            _assert_p(p, spm.p, tol=1e-3)
...
def test_anova2onerm():
    ...
    for dataset in datasets:
        ...
        x       = np.vstack( [A,B,S] ).T
        spms    = spm1d.stats.c.anova2onerm(y, x, equal_var=True).inference(0.05)
        for spm,z,df,p in zip(spms, dataset.z, dataset.df, dataset.p):
            assert z == pytest.approx(spm.z, abs=1e-2)
...
def test_anova2rm():
    ...
    for dataset in datasets:
        y,A,B,S = dataset.get_data()
        x       = np.vstack( [A,B,S] ).T
        spms    = spm1d.stats.c.anova2rm(y, x, equal_var=True).inference(0.05)
        for spm,z,df,p in zip(spms, dataset.z, dataset.df, dataset.p):
            assert z == pytest.approx(spm.z, abs=1e-2)
            assert df == pytest.approx(spm.df, abs=1e-3)
            _assert_p(p, spm.p, tol=1e-3)
...
def test_anova3():
    ...
    for dataset in datasets:
        ...
        x       = np.vstack( [A,B,C] ).T
        spms    = spm1d.stats.c.anova3(y, x, equal_var=True).inference(0.05)
        for spm,z,df,p in zip(spms, dataset.z, dataset.df, dataset.p):
            assert z == pytest.approx(spm.z, abs=1e-2)
            assert df == pytest.approx(spm.df, abs=1e-3)
            _assert_p(p, spm.p, tol=1e-3)
...
def test_anova3nested():
    ...
    for dataset in datasets:
        y,A,B,C = dataset.get_data()
        x       = np.vstack( [A,B,C] ).T
        spms    = spm1d.stats.c.anova3nested(y, x, equal_var=True).inference(0.05)
        for spm,z,df,p in zip(spms, dataset.z, dataset.df, dataset.p):
            assert z == pytest.approx(spm.z, abs=1e-2)
            assert df == pytest.approx(spm.df, abs=1e-3)
            _assert_p(p, spm.p, tol=1e-3)
...
def test_anova3onerm():
    ...
    for dataset in datasets:
        ...
        x         = np.vstack( [A,B,C,S] ).T
        spms      = spm1d.stats.c.anova3onerm(y, x, equal_var=True).inference(0.05)
        for spm,z,df,p in zip(spms, dataset.z, dataset.df, dataset.p):
...
def test_anova3rm():
    ...
    for dataset in datasets:
        y,A,B,C,S = dataset.get_data()
        x         = np.vstack( [A,B,C,S] ).T
        spms      = spm1d.stats.c.anova3rm(y, x, equal_var=True).inference(0.05)
        for spm,z,df,p in zip(spms, dataset.z, dataset.df, dataset.p):
...
def test_anova3tworm():
    ...
    for dataset in datasets:
        y,A,B,C,S = dataset.get_data()
        x         = np.vstack( [A,B,C,S] ).T
        spms      = spm1d.stats.c.anova3tworm(y, x, equal_var=True).inference(0.05)
        for spm,z,df,p in zip(spms, dataset.z, dataset.df, dataset.p):
            assert z == pytest.approx(spm.z, abs=1e-1)
            assert df == pytest.approx(spm.df, abs=1e-3)
            _assert_p(p, spm.p, tol=1e-3)
