...
def test_hotellings():
    ...
    for dataset in datasets:
        ...
        spm   = spm1d.stats.hotellings(y, mu).inference(0.05)
...
def test_hotellings_paired():
    ...
    for dataset in datasets:
        ...
        spm   = spm1d.stats.hotellings_paired(y0, y1).inference(0.05)
...
def test_hotellings2():
    ...
    for dataset in datasets:
        y0,y1 = dataset.get_data()
        spm   = spm1d.stats.hotellings2(y0, y1).inference(0.05)
        assert dataset.z == pytest.approx(spm.z, abs=1e-1)
        assert dataset.df == pytest.approx(spm.df, abs=1e-3)
        _assert_p(dataset.p, spm.p, tol=1e-3)
...
def test_cca():
    ...
    for dataset in datasets:
        ...
        spm   = spm1d.stats.cca(y, x).inference(0.05)
        assert dataset.z == pytest.approx(spm.z, abs=1e-1)
        assert dataset.df == pytest.approx(spm.df, abs=1e-3)
        _assert_p(dataset.p, spm.p, tol=1e-3)
...
def test_manova1():
    ...
    for dataset in datasets:
        ...
        spm   = spm1d.stats.manova1(y, A).inference(0.05)
        assert dataset.z == pytest.approx(spm.z, abs=1e-1)
        assert dataset.df == pytest.approx(spm.df, abs=1e-3)
        _assert_p(dataset.p, spm.p, tol=1e-3)