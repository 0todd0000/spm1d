...
import numpy as np
from spm1d.util.val.ui import val_anova1



# # 0D case
# np.random.seed(1)  # random number generator seed (for numerical reproducibility)
# JJ   = 12, 5, 5    # sample sizes
# ss   = 5, 1, 1     # standard deviations
# val0 = val_anova1(JJ, ss, niter=1000, equal_var=True)
# val1 = val_anova1(JJ, ss, niter=1000, equal_var=False)
#
# # report FPR results:
...
JJ   = 12, 5, 5    # sample sizes
ss   = 5, 1, 1     # standard deviations
val0 = val_anova1(JJ, ss, Q=101, fwhm=28, niter=1000, equal_var=True)
val1 = val_anova1(JJ, ss, Q=101, fwhm=28, niter=1000, equal_var=False)
print( f'FPR (equal_var=True):   {val0.fpr}'  )
print( f'FPR (equal_var=False):  {val1.fpr}'  )