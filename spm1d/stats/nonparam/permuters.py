...
class _PermuterTwoSample(object):
    ...
    def _update_nperm(self):
        if factorial(self.J)==np.inf:
            self.nPermTotal = -1
        else:
            s = 0.5 if self.two_tailed else 1
            self.nPermTotal = int( s * factorial(self.J) / ( factorial(self.JA)*factorial(self.JB) ))
    ...
    def build_pdf(self, iterations=-1):
        if iterations==-1:
            ...
            if self.two_tailed:
                Z           = [self.get_test_stat_ones(ones)  for i,ones in enumerate(ONES) if i<self.nPermTotal]
            else:
        ...
        if self.dim==1:
            self.ZZ         = self.Z
            if self.two_tailed:
                self.Z      = np.abs(self.Z).max(axis=1)
            else:
                self.Z      = self.Z.max(axis=1)
...
class PermuterTtest21D(_PermuterTwoSample1D):
    ...
    def get_z_critical(self, alpha=0.05):
        perc     = 100*(1-alpha)
        zstar    = np.percentile(self.Z, perc, interpolation='midpoint', axis=0)
        return zstar
